  ...
  async execute(interaction) {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        .setTitle(`${anime.title.romaji} ${anime.title.english} - Episode ${party.currentEpisode}`)
        .setDescription(cleanDescription.length > 400 ? cleanDescription.slice(0, 400) + '...' : cleanDescription)
        .setThumbnail(anime.coverImage.large)
        .addFields(
          { name: 'Total Episodes', value: anime.episodes?.toString() || 'Unknown', inline: true },
          { name: 'Current Episode', value: party.currentEpisode.toString(), inline: true }
        )
        .setColor('Green');
